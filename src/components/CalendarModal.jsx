...
const CalendarModal = ({ title, maxDays, onClose, onConfirm, selectedDates, setSelectedDates }) => {
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30">
            <div className="w-[375px] h-auto bg-white rounded-t-[20px] shadow-lg flex flex-col px-6 py-4 relative">
                ...
                <div className="flex justify-between items-center py-4">
                    <span className="text-gray-700 text-sm font-semibold">{format(currentMonth, "yyyy년 MM월")}</span>
                ...
                <button onClick={handlePrevMonth} className="absolute top-1/2 left-1 transform -translate-y-1/2 p-2">
                <svg width="8" height="15" viewBox="0 0 8 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M7 13.5L0.999999 7.5L7 1.5" stroke="#B1B5B9" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                </button>
    
                <button onClick={handleNextMonth} className="absolute top-1/2 right-1 transform -translate-y-1/2 p-2">
                <svg width="8" height="15" viewBox="0 0 8 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1 1.5L7 7.5L1 13.5" stroke="#B1B5B9" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
                </button>
                ...
                <div className="mt-2">
                    ...
                    {daysOfWeek.map((day, index) => (
                        ...
                        className={`body-semi-bold-16 min-w-[36px] ${
                            index === 0 ? "text-red" : index === 6 ? "text-[#24ABFF]" : "text-gray-3"
                        }`}
                        >
                        {day}
                        </span>
                ...
                className={`w-full py-2 mt-4 rounded-lg ${
                    selectedDates.length > 0 ? "bg-main text-dark" : "bg-gray-2 text-white"
                }`}
                >
                확인
                </button>
